...
어떤 방식이 이 설비에 맞는지 미리 고를 필요 없다. **한 신호에 4개 탐지기를 동시에 돌리고**
각각의 상태·지표를 대시보드 **탐지기 스코어보드**에 나란히 보여준다.
`method` 설정은 이제 필수가 아니라 *있으면 좋은 옵션*(참고·마법사 추천용).
...
- **공식 경고(WARNING 기록·웹훅) = OR** — 4개 중 **하나라도 발동**하면 경고. 조기감지 우선.
  경고 노트에 *어떤 탐지기가 무슨 값으로 발동했는지* 남겨 오탐 판별을 돕는다.
- 하드 ALARM(안전 트립)은 앙상블과 무관하게 항상 독립적으로 동작.
- `GET /api/status` 응답의 `detectors[]` = 각 탐지기 `{warn, metric, thr}` 스냅샷.
